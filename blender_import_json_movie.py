...
def create_joint_node(jt: Joint, prototype_scene: bpy.types.Scene) -> bpy.types.Object:
    ...
    joint_node.scale.x *= jt.radius
    joint_node.scale.y *= jt.radius
    joint_node.scale.z *= jt.radius
    return joint_node
...
def create_interval_node(intv: Interval, prototype_scene: bpy.types.Scene, object_name) -> bpy.types.Object:
    ...
    if intv.isPush:
        ...
        intv_node.scale.x *= intv.radius/2
        intv_node.scale.y *= intv.radius/2
        intv_node.scale.z *= intv.length/2
...
class ImportPretenst(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"Pretenst:  {dir}.")
        
        for _, _,files in walk(dir):
            json_files = [file for file in files if file.endswith('.json')]
            in_order = sorted(json_files, key=getint)
            for json_file in in_order:
                bpy.context.scene.render.image_settings.file_format='JPEG'
                bpy.context.scene.render.filepath = path.join(dir, json_file)
                do_import_pretenst_json(self, context, path.join(dir, json_file))
...
def do_import_pretenst_json(self: ImportPretenst, context, file_path):
    intervals, joints = load_pretenst_from_json(file_path)
    ...
    for interval in intervals:
        if (interval.isPush):
            ...
            collection.objects.link(create_interval_node(interval, prototype_scene, "Push.001"))
        else:
            collection.objects.link(create_interval_node(interval, prototype_scene, "Pull"))
    for joint in joints:
        collection.objects.link(create_joint_node(joint, prototype_scene))
